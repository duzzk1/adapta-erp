    ...
    <div class="pt-8 pb-7 flex"
        ...
        <a href="/">
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                src="{{ asset('images/logo/logo-adapta-consultas.webp') }}" alt="Logo" class="h-10 w-auto" />
            <img x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
                src="{{ asset('images/logo/logo-adapta-consultas.webp') }}" alt="Logo" class="h-8 w-auto" />
        </a>
    </div>
                ...
                @foreach ($menuGroups as $groupIndex => $menuGroup)
                    <div>
                        ...
                        <ul class="flex flex-col gap-1">
                            @foreach ($menuGroup['items'] as $itemIndex => $item)
                                <li>
                                    @php($permKey = $item['perm'] ?? null)
                                    @php($allowedItem = !$permKey || Gate::allows('use-feature', $permKey))
                                    @if (isset($item['subItems']) && $allowedItem)
                                        ...
                                        <div x-show="isSubmenuOpen({{ $groupIndex }}, {{ $itemIndex }}) && ($store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen)">
                                            <ul class="mt-2 space-y-1 ml-9">
                                                @foreach ($item['subItems'] as $subItem)
                                                    @php($subPerm = $subItem['perm'] ?? null)
                                                    @php($allowedSub = !$subPerm || Gate::allows('use-feature', $subPerm))
                                                    <li>
                                                        @if($allowedSub)

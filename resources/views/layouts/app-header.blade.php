    ...
    <div class="flex flex-col items-center justify-between grow xl:flex-row xl:px-6">
        <div
            ...
            <a href="/" class="xl:hidden">
                <img src="{{ asset('images/logo/logo-adapta-consultas.webp') }}" alt="Logo" class="h-8 w-auto" />
            </a>
        ...
        <div :class="isApplicationMenuOpen ? 'flex' : 'hidden'"
            ...
            <!-- Desktop Calls Filter (right side, next to user) -->
            @if (request()->routeIs('dashboard.calls'))
                <x-calls.filter-inline mode="desktop" />
            @endif

            <!-- User Dropdown -->
            <x-header.user-dropdown />

            {{-- Inline filter (mobile) shown on Calls dashboard inside application menu --}}
            @if (request()->routeIs('dashboard.calls'))
                <x-calls.filter-inline mode="mobile" />
            @endif
        </div>
    </div>
</header>

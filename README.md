...
The project includes a Docker setup for local development using Nginx, PHP-FPM, MySQL, and Redis.

### Prerequisites
- Docker Desktop (Windows/macOS) or Docker Engine (Linux)

### 1) Environment file
Create your environment file if not present:

```bash
copy .env.example .env
```

The example is preconfigured for Docker (`DB_HOST=db`, `REDIS_HOST=redis`).
...
- `app`: PHP-FPM for running Artisan/Composer
- `db`: MySQL 8.0 (database: `somos`, user: `somos`, password: `secret`)
- `redis`: Redis 7
...
### Frontend (Vite) in development
Run Vite inside Docker (recommended):
...
Access the app at http://localhost:8080. Vite dev server runs at http://localhost:5173.
...
Live-reload/HMR funciona automaticamente: basta editar arquivos em `resources/` e salvar.

Se o HMR não conectar:
- Confirme que http://localhost:5173 abre no navegador
- Verifique firewall liberando porta 5173
- Ajuste HMR host em [vite.config.js](vite.config.js):
	- `server: { host: true, port: 5173, hmr: { host: 'localhost' } }`
	- Se necessário, troque `'localhost'` por `127.0.0.1`
...
Remove persistent MySQL volume (optional):
...
You can also use Composer scripts to manage Docker and common tasks:
...
# Laravel inside containers
composer run app:install
composer run app:key
composer run app:migrate
composer run app:seed
composer run app:storage

# Vite dev server in Docker
composer run node:dev
composer run node:logs
```

Para gerar assets de produção (sem Vite dev):

```bash
composer run node:build
```

Depois, você pode parar o serviço `node` e acessar normalmente via Nginx.

### Shell dentro do container
Abra um shell interativo no container da aplicação:

```bash
composer run app:bash
```

Se `bash` não estiver disponível, use:
